...
if __name__ == '__main__':
    ...
    for j in range(len(casenamelist)):
        ...
        for i in range (n):
            ...
            data = sio.loadmat('logs/' +CaseName+'/MeshNO_pre_'+CaseName+'.mat')
        ...
        print('-------------------')
        print('————最大误差： %.3f(%.3f)'%(mean[:,0], std[:,0]))
        print('平均最大误差： %.3f(%.3f)'%(mean[:,1], std[:,1]))
        print('————平均误差： %.3f(%.3f)'%(mean[:,2], std[:,2]))
        print('————相对误差： %.3f(%.3f)'%(mean[:,3], std[:,3]))
